...
# """"
class CugraphEGOSampler(CugraphSampler):
    def __init__(self, num_roots, length, dn, g, _g, train_nid, num_repeat=1):
        self._g = _g
        self.num_roots, self.length = num_roots, length
        super(CugraphEGOSampler, self).__init__(dn, g, train_nid, num_roots * length, num_repeat)
    ...
    def __sample__(self):
        ...
        #print (sampled_roots)
        ego_edge_lists, seed_offsets = cugraph.batched_ego_graphs(self._g, seeds =cudf.Series(sampled_roots), radius = self.length)
        #print (ego_edge_lists['src'].append(ego_edge_lists['dst']))
        sampled_nodes = cp.asnumpy(cp.unique(ego_edge_lists['src']))
        del ego_edge_lists
        del seed_offsets
